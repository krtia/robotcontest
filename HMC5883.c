...
{
  IIC_init();
  IIC_write(0x3c,0x00,0x78);
  IIC_write(0x3c,0x01,0x40);
  IIC_write(0x3c,0x02,0x00);
}
void Multiple_read_HMC5883(unsigned char *BUF)
{
  ...
  {
      BUF[i] = IIC_read(0x3c,0x03+i);          //BUF[0]存储数据
  }
}
int HMC5883_Get_x(void)
{
  ...
  while(!(IIC_read(0x3c,0x09) & BIT0)); 
  val[1]=IIC_read(0x3c,0x03);
  val[0]=IIC_read(0x3c,0x04);
  return x;
}
int HMC5883_Get_y(void)
{
  ...
  while(!(IIC_read(0x3c,0x09) & BIT0)); 
  val[1]=IIC_read(0x3c,0x05);
  val[0]=IIC_read(0x3c,0x06);
  return y;
}
int HMC5883_Get_z(void)
{
  ...
  while(!(IIC_read(0x3c,0x09) & BIT0)); 
  val[1]=IIC_read(0x3c,0x07);
  val[0]=IIC_read(0x3c,0x08);
  return z;
}
